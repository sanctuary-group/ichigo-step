...
type FolderOption = Pick<FormFolder, "id" | "name" | "is_system">;

type PageProps = {
    form: (FormModel & { public_url?: string }) | null;
...
export default function FormEditor({
    ...
    const publish = () => {
        if (!isEdit) return;
        save(() =>
            router.post(`/forms/${model!.id}/publish`, {}, { preserveScroll: true }),
        );
    };

    const unpublish = () => {
        if (!isEdit) return;
        router.post(`/forms/${model!.id}/unpublish`, {}, { preserveScroll: true });
    };
    ...
    return (
        <>
            <Head title={isEdit ? "フォーム編集" : "フォーム作成"} />
...
FormEditor.layout = (page: React.ReactNode) => (
