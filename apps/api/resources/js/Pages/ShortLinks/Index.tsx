...
type PageProps = {
    links: ShortLinkRow[];
    stats: { total_links: number; total_clicks: number };
    filters: { q: string; sort: "clicks" | "recent" };
    baseUrl: string;
};
...
export default function ShortLinksIndex({
    ...
    const search = (e: FormEvent) => {
        ...
        router.get(
            "/short-links",
            { q: query || undefined, sort: filters.sort },
            { preserveScroll: true, preserveState: true },
        );
    };

    const setSort = (sort: "clicks" | "recent") => {
        router.get(
            "/short-links",
            { q: filters.q || undefined, sort },
            { preserveScroll: true, preserveState: true },
        );
    };

    const copy = (row: ShortLinkRow) => {
        navigator.clipboard?.writeText(`${baseUrl}/s/${row.token}`);
        setCopied(row.id);
        setTimeout(() => setCopied(null), 1500);
    };

    const remove = (row: ShortLinkRow) => {
        if (!confirm("この短縮URLを削除しますか？\nクリック計測データも失われます。"))
            return;
        router.delete(`/short-links/${row.id}`, { preserveScroll: true });
    };
...
ShortLinksIndex.layout = (page: React.ReactNode) => (
